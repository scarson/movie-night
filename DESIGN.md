...
| 2026-08-02 | **Destructive is a fourth control level, named in `control-classes.ts`** | Three hand-rolled buttons — the leave-group confirm and both delete-account controls — shared a near-identical ember string the module did not name, and one of the three omitted the inert treatment the other two carried. Latent rather than live (that button never sets `disabled`), which is exactly how the 2026-08-01 consolidation started: eight sites, five strings, nothing able to say which was canonical. Destructive is its own level rather than a recoloured secondary because it signals consequence, not rank — it is never the counterweight to a primary CTA and never appears twice on a screen. It reuses `disabledOutlinedClasses` rather than inventing a fourth inactive vocabulary, since "a disabled control becomes chrome" is one rule for every outlined level. Contrast measured, not assumed: ember carries the **boundary** at 4.70:1 on midnight and 4.12:1 on charcoal, both clear of 1.4.11's 3:1, while the label stays cream at 16.52:1 — ember never carries the label, because ember text on charcoal is that same 4.12:1 and fails the 4.5:1 floor. Hover inverts to a filled ember with a midnight label at 4.70:1, so fill and label travel together as they do in the amber pair. No rendered change: the composed strings carry the same utilities the call sites spelled out. |
| 2026-08-02 | **CTA emphasis does not vary by account state** | Considered and declined for `/tonight`, after two independent design reviews. Making the ritual primary for an empty profile rests on the claim that quick match "looks like the product working" for an account with nothing saved — which `dev/reports/first-run-experience.md` states and then discloses it never observed, since reaching that screen needs a live Anthropic call. Three things then argue against: the ritual is 3.77 screens with its Continue at y=2693 and nothing sticky (`dev/reports/mobile-qa.md`), so it is a costly place to send a new phone user; `advance()` at step 0 has no validation, so an abandoner saves five empty arrays and reaches the identical prompt having paid that scroll; and the predicate is not observable — `GET /api/user/profile` returns `emptyProfile()` for a missing row, so "no profile row" cannot be distinguished from "a row of empty lists". The real defect is upstream and is tracked there as `open-decisions.md` #14: the solo taste-map directive instructs the model to restate a taste the member never gave. Fixing the engine covers every path into it, including the abandoner, which a routing change does not. **Scope: this row is about the two entry CTAs only** — the rows below deliberately do vary the picker and the footer control by account state. |
| 2026-08-02 | A choice of one is stated, not offered | With no groups, `GroupPicker` rendered a `role="radiogroup"` holding a single pre-selected option that could not be changed. The selection is unchanged (`defaultGroupSelection` already returns solo for an empty list); only its rendering is. **A failed groups fetch keeps the picker, and the reason is not that an un-operable control is fine there** — it is the same control. It is that stating "just you tonight" asserts something the failed fetch did not establish, and the app should not make a claim it could not check. The `role="alert"` line beside it carries the rest. |
| 2026-08-02 | "Invite someone" is promoted for an account with no groups | The app's premise is two people and the only route to an invite was the smallest, lowest-contrast element on the hub. It takes the existing footer slot rather than adding one, uses `compactOutlinedButtonClasses` so the page keeps exactly one amber fill, and keys off the groups fetch the hub already makes — no new request, and no dependency on the profile question above. Suppressed when that fetch fails, since an empty list is also what failure leaves behind. |
