...
- [ ] **Every arm of a comparison gets the identical contract.** When benchmarking implementations, models or providers against each other, any difference in what each arm is *given* invalidates the result — and the failure reads as a finding about the arm rather than about the harness. **🔥 Found 2026-08-01:** a model bake-off gave the API arm a strict `response_format` JSON Schema and the subscription arms no schema at all; the subscription models free-formed the response shape and scored 0/8. Reported as-is it would have read "the frontier model fails validation 8/8 while a cheap model passes 7/8" — exactly backwards. Before believing a lopsided result, diff what each arm actually received.
- [ ] **A suspiciously total result (0/N or N/N) is a harness hypothesis before it is a finding.** Perfect failure usually means the harness never exercised the thing. Two separate 0/8 arms in one session were both harness bugs: the schema mismatch above, and `codex exec` blocking on stdin under a non-TTY background job (fix: `< /dev/null`), which produced empty transcripts that scored as model failures.
- [ ] **Raising a timeout is not a fix until the cost has been measured.** A slow test has a cause; a bigger budget only moves the threshold. **🔥 Found 2026-08-01:** a 60-click test was assumed "inherently heavy" and its budget raised 20s → 60s, and that assumption was then written into a code comment where it would outlive its author. Measured, it was 2.67s being stretched ~8x by concurrent load; the real cost was a per-click `getByRole` re-scan over a 60-item list. Fixing the scan took it to ~950ms and the budget back to 10s. Measure the isolated runtime before touching the deadline.
- [ ] **Token counts from repeated identical prompts are not a cost signal.** Providers cache prompt prefixes, so the second and later runs of the same prompt report a fraction of the first's tokens. **🔥 Found 2026-08-01:** in a 36-run effort sweep, one cell reported 9,165 tokens then 998 for byte-identical input. Use wall-clock, vary the prompt, or read cost from the provider's own accounting — never from repeated-run token totals.
...
- [ ] **Check `document.visibilityState` before believing an animation measurement.** A backgrounded tab throttles animations: `getAnimations()[0]` reports `playState: "running"` with `currentTime` frozen at `0`, so an entrance animation using `animation-fill-mode: both` computes to its `from` keyframe forever. **🔥 Found 2026-08-02:** `animate-rise-fade` read as `opacity: 0` and `translateY(8px)` 900ms into a 400ms animation, which is indistinguishable from "this content is permanently invisible" — a serious-looking bug that does not exist. `visibilityState` was `"hidden"`; a screenshot forced paint and rendered it correctly. Sample `currentTime` twice and confirm it advances, or take the screenshot.
- [ ] **A polling loop's exit condition holds vacuously on an empty set.** `jq 'all(.[]; .bucket != "pending")'` returns `true` for `[]`, so a "wait until every CI check finishes" loop exits *instantly* when no checks have been registered yet — and the next command reads a state that was never waited for. **🔥 Found 2026-08-02:** a CI wait returned immediately and `gh pr checks` then reported "no checks reported", which read as "CI is not configured for this branch". Guard the length too: `length > 0 and all(.[]; …)`.
