...
- **Phase 7 close-out review — `border-slate` on interactive boundaries (system-wide, pre-existing).** Every input, textarea and outlined control in the app uses `border-slate` on `midnight`/`charcoal`, which is ~1.5:1 — below WCAG 1.4.11's 3:1 for non-text UI components. Each control is still identifiable by its label, placeholder or glyph (all ≥4.5:1), which is how it passes today, but the boundary itself is decorative-strength. This is a Phase 6 design-system decision inherited by every surface since, not a slice-7c defect, so it was NOT changed here — a fix touches every form in the app. ~~**Phase 8 should decide** whether to promote the resting border to `ash` (6.2) or accept it explicitly.~~ **RESOLVED 2026-07-19 — must fix.** Sam set **WCAG 2.2 AA** as the project's conformance target, which removes the "accept it explicitly" option: 1.4.11 is a AA criterion. Measured precisely during handoff: slate is 1.53:1 on midnight and 1.34:1 on charcoal (worse than the ~1.5 estimate), against a 3:1 requirement, across 45 occurrences in 21 files. Remediation plan, recommended token, and the two other AA gaps found alongside it (2.4.2 duplicate page titles, 2.4.1 missing skip link) are in **`docs/accessibility.md`** — that doc, not this plan, is now authoritative for accessibility state.
- **Phase 7 close-out review — jsdom has no working `localStorage` in this setup.** `window.localStorage` is defined as an accessor on `window` but reading it yields `undefined`; `window.sessionStorage` works normally, and the real browser is fine. Any future test touching localStorage must either inject a store (as `src/lib/reduced-motion.ts` does) or `Object.defineProperty(window, "localStorage", { value, configurable: true })` (as `src/app/profile/page.test.tsx` does). Do not assume the bare global exists.
- **Phase 7 close-out review — the Browser pane's screenshot can lag the DOM.** Two separate "bugs" during visual verification (posters not rendering, the write-up showing only its first paragraph) were screenshots captured before paint; `getComputedStyle`/`getBoundingClientRect` via `javascript_tool` showed the correct state both times. **Measure with JS and treat a single screenshot as a hint, not evidence.** Separately, the pane's viewport got stuck at 863px mid-session and would not resize — narrow-width checks were done by constraining a probe container to 343px instead, which measures the same thing.
- **Phase 7 close-out review — synthetic clicks batch, real ones do not.** Two `.click()` calls in one synchronous `javascript_tool` block against a controlled list both read the *pre-click* props, so the first rating was silently discarded. This looked exactly like a state-machine bug and is not one: React batches within a task, and a human cannot produce two clicks in one. Any future in-browser interaction check must `await` between clicks.
...
- **Task 7.5:** `src/app/globals.css` gained `--person-c` (`#6fae9f`) and `--person-d` (`#b3a06a`). DESIGN.md calls for "additional person colors assigned from a curated set that maintains contrast on dark backgrounds" for groups > 2 but names only person-a/person-b, so the set did not exist. Both verified ≥6.3:1 on midnight and on charcoal, and distinct in hue from amber and ember. `personColor(i)` wraps the four.
- **Task 7.5:** the tab labels ("Taste map", "The picks", "In words") are this executor's; the plan specifies a "three-way toggle (Taste Map default)" without naming the other two. The toggle is an editorial underlined tab row, not the mockup's boxed segmented control.
- **Task 7.5:** the mockup has FOUR context-sensitive refine labels; the plan text quotes three. All four are implemented, including "Regenerate with feedback →" for feedback with no ratings.
- **Task 7.5:** the weighting line renders **only** for the viewer whose own `session.roughDay` is set, and only with 2+ members. The client can never know anyone else's flag (`getSessionForMember` serialises the requester's only), so any claim about the actual weighting could be wrong — with both members flagged, `computeWeightNote()` applies none. The copy is therefore framed as the viewer's own request ("At your request, tonight's picks lean toward everyone else. Only you can see this."), which is un-falsifiable, names nobody, and never says "rough day".
- **Task 7.5:** the kept heart fills **sage**, not amber. DESIGN.md assigns the amber-filled heart to the rough-day toggle; reusing it for "keep this pick" would collide two meanings in one glyph. Sage is the semantic success token. The remove ✕ fills ember.
- **Task 7.5:** a session with `round === 0` (created, then the match failed) gets an explicit "Find our match →" CTA rather than an automatic POST. Auto-matching on mount is exactly the behaviour the plan forbids for round 1, and the same reasoning applies here.
- **Task 7.5:** `RefinePanel` gained an `exhausted` prop. The round number only advances on a *successful* round, so a `round_limit` rejection leaves it below the ceiling and the panel cannot infer the spent budget from the count.
- **Task 7.5:** `src/lib/session-flow.ts` gained `fetchSessionResults`/`runMatchRound` and `send` now returns the error `kind` (previously only `error`). Additive; `requestMatch` and every 7.3/7.4 call site are unchanged.
- **Task 7.6 (beyond the task's single-file list):** the reduce-animations preference needed `src/lib/reduced-motion.ts` and `src/components/reduced-motion-boot.tsx` mounted in `src/app/layout.tsx`. Setting `data-reduced-motion` only from the profile page would lose the preference on any full page load, and seeding it before first paint would require an inline script — banned by `react/no-danger`. The boot component runs on hydration instead; the cost is that an entrance already in flight is cut short on the first render after a hard reload.
- **Task 7.6:** `reduced-motion.ts` takes an injectable `PreferenceStore` rather than touching `localStorage` directly. This project's vitest/jsdom exposes a `window.localStorage` getter that returns `undefined` (see Discoveries), so injection is the only way to test the real logic — and it made the no-storage and throwing-storage paths testable for real instead of mock-patched.
- **Task 7.6:** the typed delete confirmation accepts a trimmed, case-insensitive "delete" (phones capitalise and add trailing spaces). Deliberateness is preserved by requiring the whole word.
- **Phase 7 review (out of the tasks' file lists):** `parseMatchingResponse` in `src/lib/matching.ts` now keeps only the first of a repeated `tmdbId`. Nothing in the response schema stopped the model naming a film twice, and every consumer treats `tmdbId` as identity — the ranked list would have rendered two rows for one film whose keep/remove buttons shared an accessible name and toggled as one.
- **Phase 7 review (Task 7.1 file):** `src/components/poster.tsx`'s no-poster fallback initial was `text-slate` on `bg-charcoal` = 1.3:1, i.e. an empty box. Surfaced by the first recommendation in the visual harness with a null poster path; now `text-ash` (5.4).
...
**Execution Status:** ✅ SHIPPED at `32ca649` on 2026-07-19, branch `claude/app-design-plan-build-b04129`.
Slice 7a (7.1–7.2) shipped at `4f1d80a` (commits `b2d29b2`, `171b654`, `2d59cad`, `9f211bd`, `3e161d5`, `d818ff6`, `c9487a7`, `84f3afb`, `0653910`, `4f1d80a`).
Slice 7b (7.3–7.4) shipped at `e61ccee` (commits `ab710bf`, `74e715d`, `f889ad6`, `621ec45`, `e61ccee`).
Slice 7c (7.5–7.6) shipped at `32ca649` (commits `3cb4b59`, `0b37b56`, `d1ddabf`, `747c91e`, `5520acc`, `835ecc7`, `00986f2`, `8940ebc`, `d4da59b`, `32ca649`) — includes the mandated close-out review, 7 rounds.
