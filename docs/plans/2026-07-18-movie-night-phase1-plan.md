...
- **Phase 7 close-out review — `border-slate` on interactive boundaries (system-wide, pre-existing).** Every input, textarea and outlined control in the app uses `border-slate` on `midnight`/`charcoal`, which is ~1.5:1 — below WCAG 1.4.11's 3:1 for non-text UI components. Each control is still identifiable by its label, placeholder or glyph (all ≥4.5:1), which is how it passes today, but the boundary itself is decorative-strength. This is a Phase 6 design-system decision inherited by every surface since, not a slice-7c defect, so it was NOT changed here — a fix touches every form in the app. **Phase 8 should decide** whether to promote the resting border to `ash` (6.2) or accept it explicitly.
