...
// ABOUTME: Upserts user (including avatar) in D1, creates session with refresh token, and redirects to returnTo.
...
export async function GET(request: NextRequest) {
  ...
  const stateCookie = request.cookies.get("mn-oauth-state")?.value;
  const codeVerifier = request.cookies.get("mn-oauth-verifier")?.value;
  ...
  if (error) {
    ...
    response.cookies.set("mn-oauth-state", "", { ...baseCookieOpts, maxAge: 0 });
    response.cookies.set("mn-oauth-verifier", "", { ...baseCookieOpts, maxAge: 0 });
    return response;
  }
  ...
  try {
    ...
    await db
      .prepare(
        "INSERT INTO users (id, google_id, email, name, avatar_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) " +
          "ON CONFLICT(google_id) DO UPDATE SET email = excluded.email, name = excluded.name, avatar_url = excluded.avatar_url, updated_at = excluded.updated_at"
      )
    ...
    response.cookies.set("mn-session", jwt, { ...baseCookieOpts, maxAge: 900 });
    response.cookies.set("mn-refresh", refreshToken, {
      ...baseCookieOpts,
